...
### 📷 QR 인증 처리 문제
- QR 코드 인식 거리와 각도에 따라 인증 실패가 발생하는 문제가 존재
- ZXing 라이브러리를 적용하고 인식 거리를 약 6~7cm로 제한하여 인식 정확도 개선
- AForge.NET 기반 카메라 프레임 보정을 적용하여 저조도 환경에서의 인식률 향상

### 🔌 TCP/IP 통신 및 채팅 기능 문제
- 간헐적인 연결 끊김과 데이터 송수신 지연이 발생
- 연결 상태 확인 및 예외 처리 로직을 추가하여 통신 안정성 개선
- 관리자와 사용자 간 채팅 동기화 기능을 구현하여 메시지 반영 지연 문제 해결
- 채팅 내역 관리 기능을 추가하여 이전 대화 조회 가능하도록 개선
...
- QR 코드 기반 사용자 인증 시스템 구현
- 사용자 PC 메뉴 주문 기능 구현
- 관리자-사용자 실시간 채팅 기능 구현
